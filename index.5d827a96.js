const e={container:document.getElementById("container")};async function t(){let e=await fetch("https://ddragon.leagueoflegends.com/cdn/14.19.1/data/en_US/champion.json"),t=await e.json(),a=[];for(let e of Object.values(t.data)){let t=await fetch(`https://ddragon.leagueoflegends.com/cdn/14.19.1/data/en_US/champion/${e.id}.json`),n=await t.json();a.push(Object.values(n.data)[0])}return a}function a(e,t,a){let n=a.getBoundingClientRect(),i=-(t-n.y-n.height/2)/200,s=(e-n.x-n.width/2)/200;return i=Math.max(-1,Math.min(1,i)),s=Math.max(-1,Math.min(1,s)),`perspective(100px) rotateX(${i}deg) rotateY(${s}deg)`}!async function(){let n=await t();n.forEach(t=>{let a=document.createElement("div");a.classList.add("champion"),a.setAttribute("id",t.id),a.innerHTML=`
        ...
        `,e.container.appendChild(a)}),document.querySelectorAll(".champion").forEach(e=>{e.onmousemove=function(t){let n=[t.clientX,t.clientY].concat([e]);e.style.transition="transform 0.1s ease-out",window.requestAnimationFrame(function(){e.style.transform=a.apply(null,n)})},e.onmouseleave=function(){e.style.transition="transform 0.5s ease-out",e.style.transform="perspective(100px) rotateX(0deg) rotateY(0deg)"},e.addEventListener("click",()=>{let t=e.childNodes[1].childNodes[1],a=t.getAttribute("state"),i=null,s=null,l=null;for(let t=0;t<n.length;t++)if(n[t].id=e.id){i=n[t].skins;break}console.log(i);for(let e=0;e<i.length;e++)if(i[e].id===a){s=e;break}(l=i[s+1]?i[s+1]:i[0])&&(t.setAttribute("src",`https://ddragon.leagueoflegends.com/cdn/img/champion/loading/${e.id}_${l.num}.jpg`),t.setAttribute("state",l.id))})})}();
//# sourceMappingURL=index.5d827a96.js.map
